...
class DeepBeliefNet1():
    ...
    def recognize(self, true_img, true_lbl):
        ...
        print("vis--pen")
        pen_out = self.rbm_stack['vis--pen'].get_h_given_v_dir(vis)[1]

        pen = np.concatenate((pen_out, lbl), axis=1)
    ...
    def generate(self, true_lbl, name):
        ...
        pen_out = self.rbm_stack['vis--pen'].get_h_given_v_dir(random_img)[1]
        ...
        for i in range(self.n_gibbs_gener):
            ...
            pen = lbl_in[:, :-n_labels]
            vis = self.rbm_stack['vis--pen'].get_v_given_h_dir(pen)[1]
    ...
    def train_greedylayerwise(self, vis_trainset, lbl_trainset, n_iterations):
        ...
        try:
            ...
            self.loadfromfile_rbm(loc="trained_rbm", name="vis--pen")
            ...
            layer_records[0] = self.rbm_stack['vis--pen'].cd1(vis_trainset, n_iterations)
            self.savetofile_rbm(loc="trained_rbm", name="vis--pen")


            self.rbm_stack['vis--pen'].untwine_weights()
            out = self.rbm_stack['vis--pen'].get_h_given_v_dir(vis_trainset)[1]
    ...
    def train_wakesleep_finetune(self, vis_trainset, lbl_trainset, n_iterations):
        ...
        try:
            ...
            self.loadfromfile_dbn(loc="trained_dbn", name="vis--pen")
            ...
            for epoch in range(n_iterations):
                ...
                for it in range(full_swipe):
                    ...
                    wakepenprobs, wakepenstates = self.rbm_stack['vis--pen'].get_h_given_v_dir(visible_minibatch)
                    ...
                    sleepvisprobs, sleepvisstates = self.rbm_stack['vis--pen'].get_v_given_h_dir(sleeppenstates)

                    
                    psleeppenprobs, psleeppenstates = self.rbm_stack['vis--pen'].get_h_given_v_dir(sleepvisprobs)
                    pvisprobs, pvisstates = self.rbm_stack['vis--pen'].get_v_given_h_dir(wakepenstates)

                    self.rbm_stack['vis--pen'].update_generate_params(wakepenstates, visible_minibatch, pvisprobs)
                    

                    pen_states_labels = np.concatenate((wakepenstates, label_minibatch), axis=1)
                    self.rbm_stack['pen+lbl--top'].update_params(pen_states_labels, waketopstates, negpenstates, negtopstates)

                    self.rbm_stack['vis--pen'].update_recognize_params(sleepvisprobs, sleeppenstates, psleeppenstates)

                    if it % self.print_period == 0: print("(Task4.3)iteration=%7d of %7d" % (it, full_swipe))
                
                print("Epoch " + str(epoch) + " done.")

            self.savetofile_dbn(loc="trained_dbn", name="vis--pen")
