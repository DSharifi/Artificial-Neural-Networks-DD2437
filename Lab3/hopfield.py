...
def plot_energy(energy_list, attractor_energy, epochs, task3_5=False, stored_patterns=0):
    ...
    if task3_5:
        plt.title("Energy for true (fixed) and recall functions every iteration" + ", stored patterns: " + str(
            stored_patterns))
    else:
...
def task3_4(image_number=0, max_iter=25):
    ...
    for i in range(len(error_bits)):
        ...
        while error > 0.0 and epochs > 0:
            ...
            error = error / 2
            print("Error: " + str(error) + ", (" + str(error_bits[i]) + " error bits) \t epoch_number=" + str(
                max_epochs - epochs))
...
def task3_5_2(patterns=300, units=100, mode="batch", use_noise=False, distortion=0.0, bias=False):
    ...
    plt.title("Capacity of the hopfield network with " + str(patterns) + " stored patterns and " + str(
        units) + " units\n (using " + mode + " update) " + \
...
def task3_6(samples, units, activity=0.1):
    for bias in range(0,50, 5):
        ...
        for act in range(0, int(activity * 100), 1):
            ...
            counter = 0
            recalled = hopfield.recall_sparse(data, bias)
            for j in range(data.shape[0]):
                if np.array_equal(recalled[j] - data[j], np.zeros(data.shape[1])):
                    counter += 1
            stable.append(counter)
        print(stable)
        plt.plot(range(0, int(activity * 100), 1), stable, label="bias = " + str(bias))
    plt.xlabel("Average Activity (%)")
    plt.ylabel("Stable Patterns")
    plt.title("Stable Patterns depending on value Average Activity for different biases\n " + str(units) + " Units, " + str(
            samples) + " Samples, " + str(activity * 100) + " % average activity")
    plt.legend()
    plt.show()
